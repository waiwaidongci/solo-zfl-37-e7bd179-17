...
// 该格式有两类缺陷：分隔符注入碰撞、以及不同孤立代理码元都塌成 U+FFFD 碰撞。
// 这里保留它只是为了让升级前的「普通 Unicode」老记录在字段换序后仍能重放；
// 服务端匹配时会额外拦截：载荷含任何孤立代理码元则不得走该通道。
...
// 递归检测字符串中是否存在孤立代理码元（高代理后不跟低代理，或低代理无前置高代理）
export function hasLoneSurrogate(value) {
  ...
  if (value && typeof value === "object") {
    for (const k of Object.keys(value)) {
      if (hasLoneSurrogate(k) || hasLoneSurrogate(value[k])) return true;
    }
  }
  return false;
}
