...
export class Store {
  ...
  async init() {
    ...
    if (existsSync(this.legacyPath)) {
      ...
      this.db = { dbVersion: DB_VERSION, samples: seedSamples, idempotency: {}, audit: [] };
    }
    // 初次建库也要原子落盘；失败直接抛出
    await this.#persist();
  }
...
function migrate(legacy) {
  ...
  return { dbVersion: DB_VERSION, samples, idempotency: {}, audit: [{ at: nowIso(), actor: "system", action: "migrate_v1", detail: { migrated: samples.length } }] };
}
